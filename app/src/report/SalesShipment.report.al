report 70305 "TURFSales Shipment"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layouts/SalesShipment.rdlc';
    Caption = 'TT Sales Shipment';
    ...
    dataset
    {
        dataitem("Sales Shipment Header"; "Sales Shipment Header")
        {
            ...
            column(DeliveryNoteCaptionLbl; DeliveryNoteCaptionLbl)
            {
            }
            column(HeaderLine1; HeaderLine[1])
            {
            }
            column(HeaderLine2; HeaderLine[2])
            {
            }
            column(HeaderLine3; HeaderLine[3])
            {
            }
            column(HeaderLine4; HeaderLine[4])
            {
            }
            column(HeaderLine5; HeaderLine[5])
            {
            }
            column(HeaderLine6; HeaderLine[6])
            {
            }
            column(HeaderLine7; HeaderLine[7])
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(DeliveryAddressCaptionLbl; DeliveryAddressCaptionLbl)
                    {
                    }
                    ...
                    column(OrderedCaptionLbl; OrderedCaptionLbl)
                    {
                    }
                    column(ShippedCaptionLbl; ShippedCaptionLbl)
                    {
                    }
                    column(UnitsCaptionLbl; UnitsCaptionLbl)
                    {
                    }
                    column(BackorderedCaptionLbl; BackorderedCaptionLbl)
                    {
                    }
                    ...
                    dataitem("Sales Shipment Line"; "Sales Shipment Line")
                    {
                        ...
                        column(BackorderedQty; BackOrderedQty)
                        {
                        }
                        ...
                        begin
                            ...
                            UpdateQtyFields("Sales Shipment Line");
                        end;
                        ...
                        begin
                            if ShowLotSN then begin
                                ...
                                TrackingSpecCount := ItemTrackingDocMgt.RetrieveDocumentItemTracking(TempTrackingSpecBuffer, "Sales Shipment Header"."No.", DATABASE::"Sales Shipment Header", 0);
                    ...
                    dataitem(ItemTrackingLine; "Integer")
                    {
                        ...
                        begin
                            ...
                            if not ShowCorrectionLines and TempTrackingSpecBuffer.Correction then CurrReport.Skip();
                            if TempTrackingSpecBuffer.Correction then TempTrackingSpecBuffer."Quantity (Base)" := -TempTrackingSpecBuffer."Quantity (Base)";
                            ShowTotal := false;
                            if ItemTrackingAppendix.IsStartNewGroup(TempTrackingSpecBuffer) then ShowTotal := true;
                            ShowGroup := false;
                            if (TempTrackingSpecBuffer."Source Ref. No." <> OldRefNo) or (TempTrackingSpecBuffer."Item No." <> OldNo) then begin
            ...
            begin
                ...
                if not CompanyBankAccount.Get("Sales Shipment Header"."Company Bank Account Code") then CompanyBankAccount.CopyBankFieldsFromCompanyInfo(CompanyInfo);
                DimSetEntry1.SetRange("Dimension Set ID", "Dimension Set ID");
                CompanyInfo1.CalcFields(Picture);
                if not PaymentTerms.get("Payment Terms Code") then clear(PaymentTerms);
                if not ShipmentMethod.get("Shipment Method Code") then clear(ShipmentMethod);
                HeaderLine[1] := Text002;
                HeaderLine[2] := DateCaptionLbl;
                HeaderLine[3] := OurRefCaptionLbl;
                HeaderLine[4] := YourRefCaptionLbl;
                HeaderLine[5] := CustIDCaptionLbl;
                HeaderLine[6] := PaymentTermsCaptionLbl;
                HeaderLine[7] := DeliveryTermsCaptionLbl;
            end;
    ...
    var
        ...
        LinNo: Integer;
        HeaderLine: array[7] of Text[50];
        DeliveryNoteCaptionLbl: Label 'DELIVERY NOTE';
        Text002: Label 'Delivery Note:', Comment = '%1 = Document No.';
        DateCaptionLbl: Label 'Date:';
        OurRefCaptionLbl: Label 'Our Ref.:';
        YourRefCaptionLbl: Label 'YourRef.:';
        CustIDCaptionLbl: label 'Customer No.:';
        PaymentTermsCaptionLbl: Label 'Payment Terms:';
        DeliveryTermsCaptionLbl: Label 'Delivery Terms:';
        DeliveryAddressCaptionLbl: Label 'Delivery Address:';
        NoCaptionLbl: Label 'Item No.';
        DescriptionCaptionLbl: Label 'Description';
        OrderedCaptionLbl: Label 'Ordered';
        ShippedCaptionLbl: Label 'Shipped';
        UnitsCaptionLbl: Label 'Units';
        BackorderedCaptionLbl: Label 'Backordered';
        PhoneNoCaptionLbl: Label 'Phone No.:';
        HomePageCaptionLbl: Label 'Web:';
    ...
    begin
        ...
        if country.get(CompanyInfo."Country/Region Code") then begin
            i := 0;
            repeat
                i += 1;
                if CompanyAddr[i] = '' then begin
                    if CountryTranslation.get(Country.Code, SalesShipmentHeader."Language Code") then
                        CompanyAddr[1] := CountryTranslation.Name
                    else
                        CompanyAddr[i] := Country.Name;
                    Country.Code := '';
                end;
    ...
    local procedure UpdateQtyFields(var _SalesShipmentLine: Record "Sales Shipment Line")
    var
        SalesLine: Record "Sales Line";
        SalesInvLine: Record "Sales Invoice Line";
    begin
        OrderedQty := 0;
        ShippedQty := _SalesShipmentLine.Quantity;
        BackorderedQty := 0;
        SalesLine.SetRange("Document Type", SalesLine."Document Type"::Order);
        SalesLine.SetRange("Document No.", _SalesShipmentLine."Order No.");
        if SalesLine.get(SalesLine."Document Type"::Order, _SalesShipmentLine."Order No.", _SalesShipmentLine."Order Line No.") then begin
            OrderedQty := SalesLine.Quantity;
            BackorderedQty := SalesLine.Quantity - SalesLine."Quantity Shipped";
        end
        else begin
            SalesInvLine.SetCurrentKey("Order No.", "Order Line No.");
            SalesInvLine.SetRange("Order No.", _SalesShipmentLine."Order No.");
            SalesInvLine.SetRange("Order Line No.", _SalesShipmentLine."Order Line No.");
            SalesInvLine.CalcSums(Quantity);
            OrderedQty := SalesInvLine.Quantity;
            BackorderedQty := 0;
        end;
    end;
