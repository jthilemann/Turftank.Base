report 70304 "TURFReturn Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layouts/ReturnOrder.rdlc';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(PaymentTermsCpn; PaymentTermsCpn)
            {
            }
            column(ShipmentTermsCpn; ShipmentTermsCpn)
            {
            }
            column(PaymentTerms; PaymentTermsTxt)
            {
            }
            column(ShipmentTerms; ShipmentTermsTxt)
            {
            }
            column(CompanyPicture; companyinfo.Picture)
            {
            }
            column(Vendor_Authorization_No_; "Vendor Authorization No.")
            {
            }
            column(VendorAuthCpn; VendorAuthCaption)
            {
            }
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(OurRefCpn; OurRefCaption)
                    {
                    }
                    column(yourRefCpn; YourRefCaption)
                    {
                    }
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        begin
                            ...
                            while MoreLines and (TempPurchaseLine.Description = '') and (TempPurchaseLine."Description 2" = '') and (TempPurchaseLine."No." = '') and (TempPurchaseLine.Quantity = 0) and (TempPurchaseLine.Amount = 0) do MoreLines := TempPurchaseLine.Next(-1) <> 0;
                    ...
                    dataitem(VATCounterLCY; "Integer")
                    {
                        ...
                        begin
                            TempVATAmountLine.GetLine(Number);
                            VALVATBaseLCY := Round(CurrExchRate.ExchangeAmtFCYToLCY("Purchase Header"."Posting Date", "Purchase Header"."Currency Code", TempVATAmountLine."VAT Base", "Purchase Header"."Currency Factor"));
                            VALVATAmountLCY := Round(CurrExchRate.ExchangeAmtFCYToLCY("Purchase Header"."Posting Date", "Purchase Header"."Currency Code", TempVATAmountLine."VAT Amount", "Purchase Header"."Currency Factor"));
                        end;
            ...
            begin
                ...
                UpdateTermsTxt("Purchase Header");
            end;
    ...
    var
        ...
        OurRefCaption: Label 'Our Ref.:';
        PaymentTermsCpn: Label 'Payment Terms:';
        ShipmentTermsCpn: Label 'Shipment Terms:';
        PaymentTermsTxt: Text;
        ShipmentTermsTxt: Text;
        YourRefCaption: Label 'Your Ref.:';
    ...
    begin
        FormatDocument.SetTotalLabels("Purchase Header"."Currency Code", TotalText, TotalInclVATText, TotalExclVATText);
...
#pragma warning restore AA0139
    ...
    local procedure UpdateTermsTxt(PurchaseHeader: Record "Purchase Header")
    var
        PaymentTerms: Record "Payment Terms";
        ShipmentTerms: Record "Shipment Method";
    begin
        PaymentTermsTxt := '';
        ShipmentTermsTxt := '';
        if not PaymentTerms.get(PurchaseHeader."Payment Terms Code") then clear(PaymentTerms);
        PaymentTermsTxt := PaymentTerms.Description;
        if not ShipmentTerms.get(PurchaseHeader."Shipment Method Code") then clear(ShipmentTerms);
        ShipmentTermsTxt := ShipmentTerms.Description;
    end;
