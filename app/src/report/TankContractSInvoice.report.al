report 70312 "TURFTank Contract S. Invoice"
{
    RDLCLayout = './src/report/layouts/TurfTankSalesInvoiceCont.rdlc';
    Caption = 'Turf Tank Sales Invoice';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(BottomText1; BottomText[1])
            {
            }
            column(BottomText2; BottomText[2])
            {
            }
            column(BottomText3; BottomText[3])
            {
            }
            column(BottomText4; BottomText[4])
            {
            }
            column(BottomText5; BottomText[5])
            {
            }
            ...
            begin
                ...
                if "Company Bank Account Code" <> '' then begin
    ...
    var
        ...
        TermsOfPaymentLbl: label 'Terms of payment: %1 - Final payment date %2';
        PleaseTransferLbl: label 'Please transfer amount to our bank: %1';
        WhenPayingLbl: label 'When paying by bank transfer, please state %1';
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            /*CompInfo.Get();
            if CompInfo."Sort Code" <> '' then begin
                FillNameValueTable(rightheader, compinfo.FieldCaption(Name), compinfo.Name);
                FillNameValueTable(rightheader, compinfo.FieldCaption("Sort Code"), compinfo."Sort Code");
                FillNameValueTable(rightheader, compinfo.FieldCaption("Bank Account No."), compinfo."Bank Account No.");
            end else begin
                FillNameValueTable(RightHeader, CompanyInfoBankNameLbl, CompanyBankAccount.Name + ' ' + CompanyBankAccount."Bank Branch No." + ' ' + CompanyBankAccount."Bank Account No.");
    ...
    begin
        ...
        if RecRef.FieldExist(50000) then
            RespCenterTTVatNo := RecRef.Field(50000).Value;

        if RecRef.FieldExist(50005) then
            RespCenterCorpRegNo := RecRef.Field(50005).Value;
    end;
